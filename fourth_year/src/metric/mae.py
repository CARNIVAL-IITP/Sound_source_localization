...
def find_max_point(maximum_distance, num_spk, data, ):
    ...
    for j in range(num_spk):
        ...
        # near_degree_value=data[delete_degree_index]
...
def calc_mae_locata(output, target, vad, num_spk, azimuth, resolution=1, local_maximum_distance=10, calc_layer=[0], acc_threshold=5, ref_vec=90):
    ###### get voice activity region
    # vad_max=torch.max(vad, dim=1,).values.squeeze()
    # vad_True=torch.nonzero(vad_max, as_tuple=True)[0]
    ...
    for frame_num in range(output.shape[-1]):
        ...
        for layer in calc_layer:
            ...
            # local_maximum_point=find_peaks(out_frame[0, layer], distance=local_maximum_distance)[0]
            ...
            # local_max_value=out_frame[...,layer, local_maximum_point].argsort()[::-1][0, :active_spk_num]
            # local_maximum_point=local_maximum_point[local_max_value]
        ...
        # print(argmax_estimate_frame.shape)
        # exit()
        # estimate_frame=
        ...
        # print(estimate_frame.shape)
        # print(frame_azimuth.shape)
        # exit()
        ...
        # if vad.shape[1]>1:
        #     print(doa_error)
        #     print(frame_azimuth, estimate_frame)
        #     dfha=input()
        # print(doa_error[:1], doa_error[1:])
        # dfuias=input()
...
def calc_mae(output, vad, azimuth, local_maximum_distance=10, calc_layer=[0], acc_threshold=5, ref_vec=0):
    ###### get voice activity region
    # vad_max=torch.max(vad, dim=1,).values.squeeze()
    # vad_True=torch.nonzero(vad_max, as_tuple=True)[0]
    number_of_degrees_to_estimate=0
    ...
    return total_argmax_doa_error, number_of_degrees_to_estimate
