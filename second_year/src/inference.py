...
class Logger_config():
    ...
    def save_output(self, DB_type):
        try:
            ...
            DB_type=int(DB_type)
        csv=now_dict['each_file']
        pd.DataFrame(csv).to_csv(self.result_folder['inference_folder']+'/'+DB_type+'/result.csv')
        # exit()
        ...
        with open(self.result_folder['inference_folder']+'/'+DB_type+'/result.txt', 'w') as f:
            ...
            f.write('\nargmax_doa_error\n\n')
            k=np.array(csv['mae_argmax']).mean()
       
            print('MAE: {}'.format(np.round(k, 2)))
            ...
            f.write('\nsoftmax_doa_error\n\n')
            k=k=np.array(csv['mae_softmax']).mean()
            ...
            k=k=np.array(csv['mae_half_softmax']).mean()
            f.write(str(k))
            f.write('\n')
            f.write('\n\n')
    ...
    def error_update(self, DB_type, argmax_acc, softmax_acc, half_softmax_acc,argmax_doa_error, softmax_doa_error, half_softmax_doa_error,num, iter_num, pkl_file):
        now_dict=self.save_config_dict[DB_type]
        ...
        now_dict['each_file']['file_num'].append(iter_num)
        now_dict['each_file']['mae_argmax'].append(argmax_doa_error[0]/num)
        now_dict['each_file']['mae_softmax'].append(softmax_doa_error[0]/num)
        now_dict['each_file']['mae_half_softmax'].append(half_softmax_doa_error[0]/num)
        now_dict['each_file']['pkl_file'].append(pkl_file[0])
    ...
    def config(self,):
        ...
        metric_data['each_file']={}
        metric_data['each_file']['file_num']=[]
        metric_data['each_file']['mae_argmax']=[]
        metric_data['each_file']['mae_softmax']=[]
        metric_data['each_file']['mae_half_softmax']=[]
        metric_data['each_file']['pkl_file']=[]
...
class Tester():
    ...
    def test(self, ):
        ...
        for room_type in self.args['hyparam']['result_folder']['room_type']:
            ...
            with torch.no_grad():
                for iter_num, (mixed, vad, speech_azi, num_spk, pkl_file) in enumerate(tqdm(self.dataloader.test_loader, desc='Test', total=len(self.dataloader.test_loader), )):
                 
                    array_shape=self.dataloader.test_loader.dataset.args['mic_type']
                    
                    mixed=mixed.to(self.hyperparameter.device)
                    vad=vad.to(self.hyperparameter.device)
                    speech_azi=speech_azi.to(self.hyperparameter.device)

                    out, target, vad=self.model(mixed, vad, speech_azi, iter_num, 0, array_shape)
                    ...
                    self.logger.error_update(room_type, total_argmax_acc, total_softmax_acc,total_half_softmax_acc, total_argmax_doa_error, total_softmax_doa_error, total_half_softmax_doa_error,number_of_degrees_to_estimate, iter_num, pkl_file)
