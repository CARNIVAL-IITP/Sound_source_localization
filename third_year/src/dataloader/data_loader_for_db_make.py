...
import torch
import librosa
from .random_gpu_rir_generator import gpu_rir_gen, gpu_rir_gen_for_test
import scipy
import cupyx
...
import pickle
from .vad import torch_vad, torch_vad_to_wavlen
import yaml
import sklearn
...
class testset_maker_loader(datamake):
    def __init__(self, args):
        ...
        yaml_temp=open('./dataloader/room_config.yaml', 'r')
        self.room_config_yaml=yaml.safe_load(yaml_temp)
        yaml_temp.close()

        for key in self.room_config_yaml.keys():
            os.makedirs(self.pkl_dir+key, exist_ok=True)
        ...
        self.speech_csv=sklearn.utils.shuffle(self.speech_csv)[:self.args['quantity']]
        ...
        self.rir_maker=gpu_rir_gen_for_test.acoustic_simulator_on_the_fly(self.args) 

   

        
    def noise_load(self, rir_peak, noise_dir):
     
        noise_wav, fs = sf.read(self.args['noise_dir']+noise_dir, dtype='float32', )
    ...
    def make_test_set(self, idx):
        ...
        for room in self.room_config_yaml:

            now_room_config=self.room_config_yaml[room]
            ...
            rirs, mic_pos, azi_list, linear_azi_list = self.rir_maker.create_rir(now_room_config['config_dict'], num_spk=num_spk, with_coherent_noise=with_coherent_noise, mic_type=self.args['mic_type'], mic_num=self.args['mic_num'])
            ...
            if with_coherent_noise:
                ...
                noise_wav=self.noise_load(self.noise_rir_peak, now_room_config['noise'])
            ...
            save_dict['azi_linear']=torch.tensor(linear_azi_list)
            save_dict['num_spk']=num_spk
            save_dict['SNR']=coherent_noise_snr

            
            
            pkl_name=self.pkl_dir+room+'/'+str(idx)+'.pkl'
...
class val_set_maker_loader(datamake):
    def __init__(self, args):

        super(val_set_maker_loader, self).__init__() 
    ...
    def make_test_set(self, idx):
        ...
        rirs, mic_pos, azi_list, linear_azi_list = self.rir_maker.create_rir(num_spk=num_spk, with_coherent_noise=with_coherent_noise, mic_type=self.args['mic_type'], mic_num=self.args['mic_num'])
...
def val_data_maker_load(config):
    return DataLoader(val_set_maker_loader(config),
                                             pin_memory=True,
                                             **config['dataloader_dict']
                                             )
